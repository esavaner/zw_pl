...
import { ref as refDatabse, onValue, query, orderByChild, equalTo } from 'firebase/database';
...
const useDownloadImages = (artType: ART_TYPE) => {
    ...
    useEffect(() => {
        ...
        const unsub = onValue(query(databaseRef,  orderByChild('artType'), equalTo(artType)), (snapshot) => {
